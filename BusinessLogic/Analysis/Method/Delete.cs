...
namespace BusinessLogic.Analysis.Method
{
    public partial class Analysis_BL
    {
        public async Task<ActionResult> Delete(DeleteClassDTO inputModel)
        {
            try
            {
                BaseResult_VM methodResult = _account_BL.Authentication(inputModel.AuthorId, DataModel.Enum.ActionPermission.AnalysisOP_Update);
                ...
                var data = _mediator.Send(new Analysis_GetById.Query { PointerID = inputModel.id }).Result.Analysis;
                if (data == null)
                {
                    return Ok(new BaseResult_VM
                    {
                ...
                await _mediator.Send(new Analysis_DeleteById.Command { PointerID = inputModel.id });
